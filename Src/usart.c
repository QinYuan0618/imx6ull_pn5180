...
/* USER CODE BEGIN 0 */
static uint8_t		s_uart1_rxch;		/* 一个字节(字符)的中断服务处理程序 所用的暂存器 */
char				g_uart1_rxbuf[256];	/* 接收到的所有字符 */
uint8_t				g_uart1_bytes;		/* 指向实际的 buf 大小 */
/* USER CODE END 0 */
...
/* 把 printf() 的输出内容重定向到 UART 串口，这样 MCU（比如 STM32）就可以通过 串口助手看到 printf() 输出的内容 */
#ifdef __GNUC__
